...
function Studio() {
  ...
  const calibrationNext = () => mutate(() => api("/calibration/input", { method: "POST", body: JSON.stringify({ value: "" }) }));
  ...
  return <div class="shell">
    ...
    <main>
      ...
      <div class="management-grid">
        ...
        <section class="card"><div class="cardhead"><div><h3>Follower calibration</h3><p>Guided SO-ARM101 calibration with automatic backup</p></div></div><div class="toolrow"><span>{config.robot_port ? shortDevice(config.robot_port) : "Connect an arm to continue"}</span>{active === "calibrating" ? <div><button onClick={calibrationNext}>Next Step</button><button onClick={() => stop(false)}>Cancel</button></div> : <button disabled={!canPrepare || !config.robot_port} onClick={calibrate}>Start Calibration</button>}</div></section>
      </div>
