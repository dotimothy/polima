#!/usr/bin/env bash
# Compile, bundle, and deploy the newest addon + GEWB ACT checkpoint.
set -Eeuo pipefail
...
CHECKPOINT="${CHECKPOINT:-$WORKSPACE_ROOT/ACT/outputs/addon_final_gewb_2_final_combined_20260817_134913_act_20260817_134919/checkpoints/100000/pretrained_model}"
DATASET_ROOT="${DATASET_ROOT:-$WORKSPACE_ROOT/ACT/outputs/combined_datasets/addon_final_gewb_2_final_combined_20260817_134913}"
BUILD_DIR="${BUILD_DIR:-$POLIMA_OUTPUTS_ROOT/build/polima_addon_final_gewb_combined_act_100000}"
...
Usage: $(basename "$0") [options]
  ...
  --checkpoint PATH     checkpoint to export (default: newest combined ACT)
  --dataset-root PATH   LeRobot dataset used for export calibration
...
Environment variables with the uppercase names above may also override defaults.
EOF
}
...
[[ -f "$CHECKPOINT/model.safetensors" ]] || {
    echo "Missing ACT checkpoint: $CHECKPOINT" >&2
    exit 1
}
...
"$POLIMA_ROOT/bin/polima" "${compile_args[@]}" | tee "$compile_log"
